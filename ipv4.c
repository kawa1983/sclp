...
static int sclp4_xmit_skb(struct sk_buff *skb, struct sock *sk, struct dst_entry *entry)
{
    struct rtable *rt = (struct rtable*)entry;
...
static int sclp4_sendmsg(struct kiocb *iocb, struct sock *sk, struct msghdr *msg, size_t len)
{
    ...
    if (rt == NULL) {
	err = get_rtable(sk, &rt, daddr, dport);
	if (err)
	    goto out;
    ...
    err = sclp_output(sk, &rt->u.dst, dport, inet->sport, 
		      sizeof(struct iphdr), msg, len, sclp4_xmit_skb);
